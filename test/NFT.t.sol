...
import {NFT, MintPriceNotPaid, MaxSupply, ERC721TokenReceiver} from "../contracts/NFT.sol";
...
contract NFTTest is Test {
    ...
    function setUp() public {
        nft = new NFT("NFT_tutorial", "TUT", "basicUri");
    }
    ...
    function test_RevertMintMaxSupplyReached() public {
        ...
        vm.store(address(nft), loc, mockedCurrentTokenId); // modify this slot's value of the contract storage.
    ...
    function test_NewMintOwnerRegistered() public {
        ...
        uint160 ownerOfTokenIdOne = uint160(
            uint256(
                (vm.load(address(nft), bytes32(abi.encode(slotOfNewOwner)))) // read this slot's value of the contract storage.
            )
        );
    ...
    function test_BalanceIncremented() public {
        nft.mintTo{value: 0.08 ether}(address(1));
        uint256 slotBalance = stdstore.target(address(nft)).sig(nft.balanceOf.selector).with_key(address(1)) // tokens' amount.
            .find();

        uint256 balanceFirstMint = uint256(
            vm.load(address(nft), bytes32(slotBalance)) // read storage slot's value
        );
    ...
    function test_RevertUnSafeContractReceiver() public {
        ...
        vm.etch(address(11), bytes("mock code")); // set an address some code, these code is unavailiable
        vm.expectRevert(bytes("")); // Revert with some messages.
    ...
    function test_WithdrawalFailsAsNotOwner() public {
        ...
        vm.startPrank(address(0xd3ad)); // pranks with a scope
