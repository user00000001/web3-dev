...
describe("OurToken Unit Test", () =>{
    ...
    let ourToken: OurToken, deployer: string, user1: string;
    beforeEach(async function () {
        const accounts = await getNamedAccounts();
        deployer = accounts.deployer
        user1 = accounts.user;
        await deployments.fixture(["erc20"]);
        ourToken = await ethers.getContract("OurToken1", deployer);
    });
    ...
    describe("allowances", () => {
        ...
        beforeEach(async () => {
            playerToken = await ethers.getContract("OurToken1", user1)
        })
